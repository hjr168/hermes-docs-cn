...
本指南介绍如何为 Hermes 网关添加新的消息平台。平台适配器将 Hermes 连接到外部消息服务（Telegram、Discord、企业微信等），使用户可以通过该服务与代理交互。

:::tip
添加平台适配器涉及 20+ 个文件，涵盖代码、配置和文档。请将本指南作为检查清单使用——适配器文件本身通常只占工作量的 40%。
:::
...
2. **`load_gateway_config()`** — 添加 token 环境变量映射：`Platform.NEWPLAT: "NEWPLAT_TOKEN"`
3. **`_apply_env_overrides()`** — 映射所有 `NEWPLAT_*` 环境变量到配置
...
**`agent/prompt_builder.py`** — 如果你的平台有特定的渲染限制（不支持 Markdown、消息长度限制等），在 `_PLATFORM_HINTS` 字典中添加一个条目。这会将平台特定的指导注入系统提示：
...
在将新的平台 PR 标记为完成之前，对照一个已有的平台进行对等审计：
...
# 第一个集合中有但第二个集合中没有的任何文件都是潜在的遗漏
```

对 `.md` 和 `.ts` 文件重复此操作。调查每个遗漏——它是平台枚举（需要更新）还是平台特定的引用（可以跳过）？
...
如果平台向你的端点推送消息（如企业微信回调），运行一个 HTTP 服务器：
...
对于有严格响应时限的平台（例如企业微信的 5 秒限制），始终立即确认，稍后通过 API 主动投递代理的回复。代理会话运行时间为 3-30 分钟——在回调响应窗口内进行内联回复是不可行的。

### Token 锁

如果适配器持有使用唯一凭证的持久连接，添加作用域锁以防止两个配置文件使用相同的凭证：
...
| `telegram.py` | 长轮询 + Bot API | 高 | 功能完整的适配器，支持群组、线程 |

---
> 📝 本文由 AI 翻译，如有疑问请参考[英文原版](/docs/developer-guide/adding-platform-adapters)
