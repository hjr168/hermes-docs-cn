...
- 可选的自动清理：当 `sessions.auto_prune` 为 `true` 时，超过 `sessions.retention_days`（默认 90 天）的已结束会话会在 CLI/网关启动时被清理
- 清理后如果实际删除了行，`state.db` 会执行 `VACUUM` 以回收磁盘空间（SQLite 在普通 DELETE 后不会缩小文件）
- 清理最多每 `sessions.min_interval_hours`（默认 24 小时）运行一次；最后运行时间戳存储在 `state.db` 本身中，因此在同一 `HERMES_HOME` 中的所有 Hermes 进程共享
- 默认**关闭**——会话历史对 `session_search` 回溯很有价值，静默删除可能会让用户感到意外。在 `~/.hermes/config.yaml` 中启用：

```yaml
sessions:
  auto_prune: true          # 可选——默认关闭
  retention_days: 90        # 保留已结束会话的天数
  vacuum_after_prune: true  # 清理后回收磁盘空间
  min_interval_hours: 24    # 清理扫描的最短间隔
```
...
:::tip
数据库增长缓慢（典型：数百个会话占用 10-15 MB），会话历史为跨对话的 `session_search` 回溯提供支持，因此自动清理默认关闭。如果你在运行繁重的网关/Cron 工作负载，其中 `state.db` 明显影响性能（观察到的失败模式：~1000 个会话的 384 MB state.db 拖慢 FTS5 插入和 `/resume` 列表），请启用它。使用 `hermes sessions prune` 进行一次性清理，而无需开启自动扫描。
:::
