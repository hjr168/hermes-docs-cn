...
sidebar_position: 8
title: "扩展 CLI"
description: "构建包装器 CLI，通过自定义小部件、键绑定和布局变更扩展 Hermes TUI"
---

# 扩展 CLI

Hermes 在 `HermesCLI` 上暴露了受保护的扩展钩子，让包装器 CLI 可以添加小部件、键绑定和布局自定义，而无需覆盖 1000+ 行的 `run()` 方法。这使你的扩展与内部变更解耦。
...
| 钩子 | 用途 | 何时覆盖... |
|------|------|------------|
| `_get_extra_tui_widgets()` | 向布局注入小部件 | 你需要持久的 UI 元素（面板、状态行、迷你播放器） |
...
返回要插入到 TUI 布局中的 prompt_toolkit 小部件列表。小部件出现在**间隔器和状态栏之间** — 在输入区域上方但在主输出下方。
...
每个小部件应该是一个 prompt_toolkit 容器（如 `Window`、`ConditionalContainer`、`HSplit`）。使用 `ConditionalContainer` 或 `filter=Condition(...)` 使小部件可切换。
...
在 Hermes 注册自己的键绑定之后、构建布局之前调用。将你的键绑定添加到 `kb`。
...
仅在需要完全控制小部件排序时覆盖。大多数扩展应该使用 `_get_extra_tui_widgets()` 代替。
...
- **斜杠命令**：覆盖 `process_command()`，处理你的命令，其他命令调用 `super().process_command(cmd)`。
- **除非绝对必要，不要覆盖 `run()`** — 扩展钩子的存在正是为了避免这种耦合。

---
> 📝 本文由 AI 翻译，如有疑问请参考[英文原版](/docs/developer-guide/extending-the-cli)
