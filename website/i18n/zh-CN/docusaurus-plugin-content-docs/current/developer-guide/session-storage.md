...
关键设计决策：
- **WAL 模式**用于并发读取器 + 一个写入器（Gateway 多平台）
...
多个 Hermes 进程（Gateway + CLI 会话 + 工作树 Agent）共享一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
---
> 📝 本文由 AI 翻译，如有疑问请参考[英文原版](/docs/developer-guide/session-storage)
