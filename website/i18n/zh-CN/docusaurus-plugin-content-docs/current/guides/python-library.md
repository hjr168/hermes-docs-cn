...
description: "在你自己的 Python 脚本、Web 应用或自动化管道中嵌入 AIAgent — 无需 CLI"
---
...
使用 `ephemeral_system_prompt` 设置自定义系统提示来引导 Agent 行为，但**不会**保存到轨迹文件（保持训练数据干净）：
...
:::warning
始终为每个线程或任务创建**新的 `AIAgent` 实例**。Agent 维护的内部状态（对话历史、工具会话、迭代计数器）不适合跨线程共享。
:::
...
---
> 📝 本文由 AI 翻译，如有疑问请参考[英文原版](/docs/guides/python-library)
