...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置完整迁移到 Hermes Agent 的指南 — 迁移内容、配置映射和迁移后检查清单"
---
...
迁移总是先显示完整的导入预览，确认后才会执行更改。查看列表后确认即可。

默认从 `~/.openclaw/` 读取。旧版的 `~/.clawdbot/` 或 `~/.moltbot/` 目录会自动检测。旧版配置文件名（`clawdbot.json`、`moltbot.json`）同样自动识别。
...
| `--migrate-secrets` | 包含 API 密钥（使用 `--preset full` 时默认启用）。 |
...
工作区文件也会检查 `workspace.default/` 和 `workspace-main/` 作为备用路径（OpenClaw 在近期版本中将 `workspace/` 重命名为 `workspace-main/`，并使用 `workspace-{agentId}` 支持多 Agent 设置）。

### Skill（4 个来源）

| 来源 | OpenClaw 位置 | Hermes 目标 |
|------|--------------|-------------|
| 工作区 Skill | `workspace/skills/` | `~/.hermes/skills/openclaw-imports/` |
| 托管/共享 Skill | `~/.openclaw/skills/` | `~/.hermes/skills/openclaw-imports/` |
| 个人跨项目 Skill | `~/.agents/skills/` | `~/.hermes/skills/openclaw-imports/` |
| 项目级共享 Skill | `workspace/.agents/skills/` | `~/.hermes/skills/openclaw-imports/` |

Skill 冲突通过 `--skill-conflict` 处理：`skip` 保留现有 Hermes Skill，`overwrite` 替换它，`rename` 创建一个 `-imported` 副本。
...
| 自定义提供商 | `models.providers.*` | `config.yaml` → `custom_providers` | 映射 `baseUrl`、`apiType`/`api` — 同时处理短格式（"openai"、"anthropic"）和连字符格式（"openai-completions"、"anthropic-messages"、"google-generative-ai"） |
| 提供商 API 密钥 | `models.providers.*.apiKey` | `~/.hermes/.env` | 需要 `--migrate-secrets`。参见下方 [API 密钥解析](#api-密钥解析)。 |
...
| 拟人延迟 | `agents.defaults.humanDelay.mode` | `human_delay.mode` | "natural" / "custom" / "off" |
| 拟人延迟时间 | `agents.defaults.humanDelay.minMs` / `.maxMs` | `human_delay.min_ms` / `.max_ms` | 直接复制 |
| 时区 | `agents.defaults.userTimezone` | `timezone` | 直接复制字符串 |
| 执行超时 | `tools.exec.timeoutSec` | `terminal.timeout` | 直接复制（字段名是 `timeoutSec`，不是 `timeout`） |
...
注意：OpenClaw 还有 `session.resetTriggers`（简单字符串数组，如 `["daily", "idle"]`）。如果结构化的 `session.reset` 不存在，迁移会回退到从 `resetTriggers` 推断。
...
|------|------------------|-------------------|------|
| Telegram | `channels.telegram.botToken` 或 `.accounts.default.botToken` | `TELEGRAM_BOT_TOKEN` | Token 可以是字符串或 [SecretRef](#secretref-处理)。扁平布局和 accounts 布局都支持。 |
...
这些会保存到 `~/.hermes/migration/openclaw/<timestamp>/archive/` 供手动审查：
...
| `BOOTSTRAP.md` | `archive/workspace/BOOTSTRAP.md` | 使用上下文文件或 Skill |
...
| 钩子/Webhook | `archive/hooks-config.json` | 使用 `hermes webhook` 或 Gateway 钩子 |
| 记忆后端 | `archive/memory-backend-config.json` | 通过 `hermes honcho` 配置 |
| Skill 注册表 | `archive/skills-registry-config.json` | 使用 `hermes skills config` |
...
启用 `--migrate-secrets` 时，API 密钥从**四个来源**按优先级收集：

1. **配置值** — `openclaw.json` 中的 `models.providers.*.apiKey` 和 TTS 提供商密钥
2. **环境文件** — `~/.openclaw/.env`（如 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等密钥）
3. **配置 env 子对象** — `openclaw.json` → `"env"` 或 `"env"."vars"`（某些安装将密钥存储在此而非单独的 `.env` 文件中）
4. **认证档案** — `~/.openclaw/agents/main/agent/auth-profiles.json`（每个 Agent 的凭证）
...
OpenClaw 中 Token 和 API 密钥的配置值可以有三种格式：
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，它会从 `~/.openclaw/.env` 和 `openclaw.json` env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析 — 迁移会发出警告，这些值需要通过 `hermes config set` 手动添加。

## 迁移后

1. **检查迁移报告** — 完成时打印，包含已迁移、跳过和冲突项目的计数。

2. **审查归档文件** — `~/.hermes/migration/openclaw/<timestamp>/archive/` 中的内容需要手动处理。

3. **启动新会话** — 导入的 Skill 和记忆条目在新会话中生效，不会在当前会话中生效。
...
7. **重新配对 WhatsApp** — WhatsApp 使用 QR 码配对（Baileys），不是 Token 迁移。运行 `hermes whatsapp` 进行配对。
...
迁移依次检查 `~/.openclaw/`、`~/.clawdbot/`、`~/.moltbot/`。如果你的安装在其他位置，使用 `--source /path/to/your/openclaw`。

### "No provider API keys found"

密钥可能存储在多个位置，取决于你的 OpenClaw 版本：内联在 `openclaw.json` 的 `models.providers.*.apiKey` 中、在 `~/.openclaw/.env` 中、在 `openclaw.json` 的 `"env"` 子对象中，或在 `agents/main/agent/auth-profiles.json` 中。迁移会检查所有四个位置。如果密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析 — 通过 `hermes config set` 手动添加。

### 迁移后 Skill 未出现

导入的 Skill 位于 `~/.hermes/skills/openclaw-imports/`。启动新会话使其生效，或运行 `/skills` 验证它们是否已加载。

### TTS 语音未迁移

OpenClaw 在两个位置存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会检查两者。如果你的语音 ID 是通过 OpenClaw UI 设置的（存储在不同路径），可能需要手动设置：`hermes config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。

---
> 📝 本文由 AI 翻译，如有疑问请参考[英文原版](/docs/guides/migrate-from-openclaw)
